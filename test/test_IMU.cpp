...
void TestIMUFuctions(){
    ...
    ScaViSLAM::strapdown_local_quat_bias(rvqs0.head(3), rvqs0.segment(3,3), quat,a, w, dt,
    ...
    sys_local_dcm_bias(rvqs0.head(3), rvqs0.segment(3,3), quat,
...
void TestIMURoutine(char * argv[])
{
    ...
    SE3 T_s_to_c(tempRs2c, tempVec3d);
    SE3 T_imu_from_cam=T_s_to_c.inverse();
    T_s1_to_w= T_s_to_c;
    ...
    SE3 temp_Tc2w=T_s1_to_w*T_imu_from_cam;
    ...
    strapdown_local_quat_bias( r_old, v_old, q_old, est_measurement.head(3),
                               est_measurement.tail(3),
    ...
    if(predict_cov)
    {
        sys_local_dcm_bias(r_old, v_old, q_old, est_measurement.head(3),est_measurement.tail(3),
                           dt,q_n_aw_babw.head(3), q_n_aw_babw.segment(3,3),q_n_aw_babw.segment(6,3),q_n_aw_babw.tail(3),&P);
    }
    r_old=r_new;
    v_old=v_new;
    q_old=q_new;
    ...
    for (; i<measurements.size()-1;++i){
        ...
        {
            sys_local_dcm_bias(r_old, v_old, q_old,est_measurement.head(3),
                               est_measurement.tail(3),
                               dt,q_n_aw_babw.head(3), q_n_aw_babw.segment(3,3),q_n_aw_babw.segment(6,3),q_n_aw_babw.tail(3),&P);
        }
        r_old=r_new;
