...
//#include <opencv2/features2d/features2d.hpp> //for drawmatches
...
bool VisualOdometryStereo::process (uint8_t *I1,uint8_t *I2,int32_t* dims,bool replace) {
  ...
  // Based on my implementation and some preliminary test, Klein's method gives similar result to Geiger's method,
  // both obtained better results than Badino's method. In terms of speed, Geiger and Klein's method are similar and faster than Badino's method
  // so there is not much benefit of using alternative methods to Geiger's implementation
...
//  cout<<p_matched.size()<<" after bucketing!"<<endl;
  ...
  break;
  case ROBUST_Badino:
      tr_delta=estimateMotionBadino(p_matched);
      break;
  case ROBUST_Klein:
...
vector<double> VisualOdometryStereo::estimateMotionBadino (const vector<p_match>& p_matched) {
...
//estimate motion using Badino's robust estimation method discussed in A head wearable short baseline stereo system...

  for (int32_t i=0; i<(int32_t)p_matched.size(); i++)
    inliers.push_back(i);
  vector<int32_t> active=inliers;
  double rmax2(-1);
  VisualOdometryStereo::result result = UPDATED;
  // initial ROBUST estimate, use ransac_iters to limit iterations
  for (int32_t k=0;k<param.ransac_iters;k++) {
    if(result==CONVERGED && inliers.size()>=6)
        break;
    tr_delta_curr = tr_delta;
    // minimize reprojection errors

    int32_t iter=0;
    while (result==UPDATED) {
      result = updateParameters(p_matched,inliers,tr_delta_curr,1,1e-6);
      if (iter++ > 20 || result==CONVERGED)
        break;
    }
    // compute E_k
    double Ek(0);
    for (int32_t i=0; i<(int32_t)inliers.size(); i++)
      Ek+=(pow(p_observe[4*i+0]-p_predict[4*i+0],2)+pow(p_observe[4*i+1]-p_predict[4*i+1],2) +
    ...
    if (result!=FAILED) {
      ...
      // extract observations and compute predictions for all matches
      computeObservations(p_matched,active);
      computeResidualsAndJacobian(tr_delta_curr,active);

      // determine new inliers based on E_k
      inliers.clear();
      for (int32_t i=0; i<(int32_t)p_matched.size(); i++)
        if (pow(p_observe[4*i+0]-p_predict[4*i+0],2)+pow(p_observe[4*i+1]-p_predict[4*i+1],2) +
            pow(p_observe[4*i+2]-p_predict[4*i+2],2)+pow(p_observe[4*i+3]-p_predict[4*i+3],2) < rmax2)
          inliers.push_back(i);
    }
  }
  // final optimization (refinement)
...
vector<int32_t> VisualOdometryStereo::getInlier(const vector<p_match> &p_matched,vector<double> &tr) {
...
VisualOdometryStereo::result VisualOdometryStereo::updateParameters(const vector<p_match> &p_matched,vector<int32_t> &active,vector<double> &tr,double step_size,double eps) {
...
void VisualOdometryStereo::computeObservations(const vector<p_match> &p_matched,vector<int32_t> &active) {
